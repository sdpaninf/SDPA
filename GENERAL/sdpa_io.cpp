	      ...
	      BlockStruct& bs, bool inputSparse)
{
  ...
  if (inputSparse) {
    ...
    double value;
    while (true) {
      if (fscanf(fpData,"%*[^0-9+-]%ld",&target)<=0) {
	break;
      }
      if (fscanf(fpData,"%*[^0-9+-]%ld",&l)<=0) {
	break;
      }
      if (fscanf(fpData,"%*[^0-9+-]%ld",&i)<=0) {
	break;
      }
      if (fscanf(fpData,"%*[^0-9+-]%ld",&j)<=0) {
	break;
      }
  ...
  if (isDataSparse) {
	...
	break;
      ...
      if (bs.blockType[l-1] == BlockStruct::btSDP) {
        ...
        if (i!=j){
          printf("invalid data file k:%ld, l:%ld, i:%ld, j:%ld, value:%lf\n",
                 k,l,i,j,value);
