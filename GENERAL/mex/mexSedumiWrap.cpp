		...
		mxArray* info_ptr)
{
  ...
  if (field_ptr != NULL) {
    K_l   = (size_t)((mxGetPr(field_ptr))[0]);
    #if 0
    mexPrintf("K_l = %zd\n", K_l);
    #endif
    if (K_l > 0) {
  ...
  size_t* K_s             = NULL;
  size_t* K_sdpConeStart  = NULL;
  int     K_sdpNoCones    = 0;
  field_ptr = mxGetField(K_ptr, 0, "s");
  if (field_ptr != NULL) {
    K_sdpNoCones = (int) mxGetM(field_ptr);
    K_s = (size_t*) mxCalloc(K_sdpNoCones, sizeof(size_t));
    K_sdpConeStart = (size_t*) mxCalloc(K_sdpNoCones+1, sizeof(size_t));
    K_sdpConeStart[0] = 0;
    for (int l=0; l<K_sdpNoCones; ++l) {
      K_s[l] = (size_t)((mxGetPr(field_ptr))[l]);
      #if 0
      mexPrintf("K_s[%zd] = %zd\n", l+isK_l+1, K_s[l]);
      #endif
  ...
  for (k=0; k<mDIM; ++k) {
    ...
    int currentSdpCone = 0;
    for (size_t index = Ak_start; index<Ak_end; ++index) {
      size_t Ak_j =  At_row[index];
      /* F_{k+1} = - A_k */
