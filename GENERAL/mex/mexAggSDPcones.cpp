...
//    std::cout << "A0jc = ";
//    for (unsigned int j=0; j <= A0colSize; j++) 
//        std::cout << A0jc[j] << " "; 
//    std::cout << std::endl;
    mwIndex* A0ir = mxGetIr(A0_ptr);
//    std::cout << "A0ir = ";
//    for (unsigned int i=0; i < A0nnz; i++) 
//        std::cout << A0ir[i] << " "; 
//    std::cout << std::endl;            
...
//    std::cout << "c0jc = ";
//    for (unsigned int j=0; j <= c0colSize; j++) 
//        std::cout << c0jc[j] << " "; 
//    std::cout << std::endl;    
    mwIndex* c0ir = mxGetIr(c0_ptr);
//    std::cout << "c0ir = ";
//    for (unsigned int i=0; i < c0nnz; i++) 
//        std::cout << c0ir[i] << " "; 
//    std::cout << std::endl;
...
//    std::cout << "A1colSize = " << A1colSize << std::endl;
    ...
    int topNDblks = 0; 
    int bottomNDblks = 0; 
    unsigned int A0colPt = 0;
    unsigned int A1colPt = 0; 
    A1jc[A1colPt] = 0; 
    c1jc[A1colPt] = 0; 
    for (unsigned int k0 = 0; k0 < K0sSize; k0++) {
...
//        std::cout << " k0 = " << k0 << " topNDblks = " << topNDblks << " bottomNDblks =  " << bottomNDblks << std::endl;
        for (unsigned int j0 = 0; (int) j0 < K0spr[k0]; j0++) {
            if (topNDblks > 0) {
                for (unsigned int k1 = 0; (int) k1 < topNDblks; k1++) {
            ...
            if (bottomNDblks > 0) {
                for (unsigned int k1 = 0; (int) k1 < bottomNDblks; k1++) {
                    A1jc[A1colPt+1] = A1jc[A1colPt];
                    c1jc[A1colPt+1] = c1jc[A1colPt];
                    A1colPt++;
                }
...
//    std::cout << "A1jc =  ";
//    for (unsigned int k1 = 0; k1 <= A1colSize; k1++)
//        std::cout << A1jc[k1] << "  "; 
//    std::cout << std::endl;        
    /* A1ir = A0ir and A1pr = A0pr */
    for (unsigned int p=0; p < A0nnz; p++) {
        A1ir[p] = A0ir[p];
        A1pr[p] = A0pr[p];
    }
    
    /* c1ir = c0ir and c1pr = c0pr */
    for (unsigned int p=0; p < c0nnz; p++) {
        c1ir[p] = c0ir[p];
        c1pr[p] = c0pr[p];
    }
