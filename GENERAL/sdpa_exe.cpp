...
static void argumentAnalysis(SDPA& Problem1,
			     ...
			     bool& isDimacs, SDPA_INT& numThreads)
{
  if (argc == 1) {
    message(argv[0]);
  }
  if (argv[1][0] == '-') {
    ...
    for (SDPA_INT index = 0; index < argc; ++index) {
	...
	SDPA_INT tmp = atoi(argv[index+1]);
	switch (tmp) {
	case 0:
    ...
    SDPA_INT len = strlen(inputFileName);
    ...
    for (SDPA_INT index=3; index<argc; ++index) {
	...
	SDPA_INT len = strlen(initFileName);
...
int main(int argc, char** argv)
{
  ...
  bool isDimacs = false;
  SDPA_INT  numThreads = 0; // 0 means automatic computation
