...
void SDPA::initializeSolve()
{
  ...
  SDPA_INT nBlock2 = bs.SDP_nBlock + bs.SOCP_nBlock + bs.LP_nBlock;
...
void SDPA::solve()
{
  if (isInitPoint == true) {
    ...
    FILE* thetaFP = NULL;
    thetaFP = fopen("thetaFP.txt","r");
    if (thetaFP != NULL) {
      //      rMessage("read thetaFP.txt");                                                                                              
      fscanf(thetaFP,"%lf,%lf,%ld",&theta.primal,&theta.dual,&pIteration);
      fclose(thetaFP);
    }
  }
  ...
  while (phase.updateCheck(currentRes, solveInfo, param)
...
#if 0 // 2007/08/29 kazuhide nakata
    ...
    INIT_FILE_NAME[0] = '\0';
    sprintf(INIT_FILE_NAME, "sdpara.init");
    fpinitf = fopen(INIT_FILE_NAME,"w");
    IO::printSolution(bs, currentPt, param, fpinitf);
    fclose(fpinitf);
#if 0
    sprintf(INIT_FILE_NAME, "sdpara.init.%ld", pIteration);
    fpinitf = fopen(INIT_FILE_NAME,"w");
    IO::printSolution(bs, currentPt, param, fpinitf);
    fclose(fpinitf);
    sprintf(INIT_FILE_NAME, "sdpara.comptime.%ld", pIteration);
    fpinitf = fopen(INIT_FILE_NAME,"w");
    com.display(fpinitf);
    fclose(fpinitf);
#endif
    
    FILE* thetaFP = fopen("thetaFP.txt","w");
    fprintf(thetaFP,"%8.16e,%8.16e,%ld",theta.primal,theta.dual,pIteration);
    fclose(thetaFP);
#if 0
    sprintf(INIT_FILE_NAME, "thetaFP.txt.%ld", pIteration);
    thetaFP = fopen(INIT_FILE_NAME,"w");
    //fpinitf = fopen(INIT_FILE_NAME,"w"); // Endo debug                                                                                 
    fprintf(thetaFP,"%8.16e,%8.16e,%ld",theta.primal,theta.dual,pIteration);
    fclose(thetaFP);
#endif
  } // end of MAIN_LOOP
  ...
  sprintf(INIT_FILE_NAME, "thetaFP.txt.%ld", pIteration);
  fpinitf = fopen(INIT_FILE_NAME,"w");
  fprintf(fpinitf,"%8.16e,%8.16e,%ld",theta.primal,theta.dual,pIteration);
  fclose(fpinitf);
#endif
