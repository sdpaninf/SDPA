...
{
  SDPA_INT nBlock = aVec.nBlock;
  double ret = aVec.ele[0].ele[0];
  double tmp;
  SDPA_INT size = aVec.ele[0].nDim;
  for (SDPA_INT j=1; j<size; ++j) {
    tmp = aVec.ele[0].ele[j];
    if (tmp < ret) {
      ret = tmp;
    }
  }
  for (SDPA_INT k=1; k<nBlock; ++k) {
				   ...
				   ComputeTime& com)
{
  ...
  if (-minxInvDxEigenValue > 1.0 /alphaBD) {
    ...
    // the limlong int of primal steplength
  } else {
    primal = alphaBD;
  }
  TimeEnd(END1);
  ...
  if (-minzInvDzEigenValue > 1.0 /alphaBD) {
    ...
    // the limlong int of dual steplength
  } else {
    dual = alphaBD;
  }
  TimeEnd(END2);
		       ...
		       Parameter& param)
{
  ...
  for (SDPA_INT l=0; l<currentPt.xMat.SDP_nBlock; l++){
    SDPA_INT dim = currentPt.xMat.SDP_block[l].nRow; 
    for (SDPA_INT k=0; k<dim; k++){
      x0zMat += lambda * currentPt.zMat.SDP_block[l].de_ele[k*dim+k];
      xMatz0 += lambda * currentPt.xMat.SDP_block[l].de_ele[k*dim+k];
    }
  }
  for (SDPA_INT l=0; l<currentPt.xMat.SOCP_nBlock; l++){
    rError("no support for SOCP");
  }
  for (SDPA_INT l=0; l<currentPt.xMat.LP_nBlock; l++){
...
// prlong int information of ObjVal, residual, gap, complementarity
