...
Solutions::Solutions(SDPA_INT m, BlockStruct& bs,
		     double lambda,ComputeTime& com)
{
  initialize(m, bs, lambda, com);
}

void Solutions::initialize(SDPA_INT m, BlockStruct& bs,
			   double lambda, ComputeTime& com)
{
  mDim = m;
  nDim = 0;
  for (SDPA_INT l=0; l<bs.SDP_nBlock; ++l) {
    nDim += bs.SDP_blockStruct[l];
  }
  for (SDPA_INT l=0; l<bs.SOCP_nBlock; ++l) {
...
void Solutions::initializeZero(SDPA_INT m, BlockStruct& bs,
			       ComputeTime& com)
{
...
void InputData::initialize_bVec(SDPA_INT m)
{
  b.initialize(m);
}
...
void InputData::terminate()
{
...
#if 0
  if (SOCP_nConstraint && SOCP_constralong int && SOCP_blockIndex){
    for (SDPA_INT k=0; k<SOCP_nBlock; ++k) {
  ...
  if (LP_nConstraint && LP_constraint && LP_blockIndex){
    for (SDPA_INT k=0; k<LP_nBlock; ++k) {
...
{
  SDPA_INT mDim = b.nDim;
  SDPA_INT index;
  SDPA_INT* SDP_count;

  NewArray(SDP_nConstraint,SDPA_INT,SDP_nBlock);

  // count non-zero block matrix of A
  for (SDPA_INT l=0; l<SDP_nBlock; l++){
    SDP_nConstraint[l] = 0;
  }
  for (SDPA_INT k=0; k<mDim; k++){
    for (SDPA_INT l=0; l<A[k].SDP_sp_nBlock; l++){
      index = A[k].SDP_sp_index[l];
      SDP_nConstraint[index]++;
    }
  }
  // malloc SDP_constraint, SDP_blockIndex
  NewArray(SDP_constraint,SDPA_INT*,SDP_nBlock);
  for (SDPA_INT l=0; l<SDP_nBlock; l++){
    NewArray(SDP_constraint[l],SDPA_INT,SDP_nConstraint[l]);
  }
  NewArray(SDP_blockIndex,SDPA_INT*,SDP_nBlock);
  for (SDPA_INT l=0; l<SDP_nBlock; l++){
    NewArray(SDP_blockIndex[l],SDPA_INT,SDP_nConstraint[l]);
  }
  // input index of non-zero block matrix of A
  NewArray(SDP_count,SDPA_INT,SDP_nBlock);
  for (SDPA_INT l=0; l<SDP_nBlock; l++){
      SDP_count[l] = 0;
  }
  for (SDPA_INT k=0; k<mDim; k++){
...
void InputData::initialize_index_SOCP()
{
  ...
  NewArray(SOCP_nConstraint,SDPA_INT,SOCP_nBlock);
  ...
  for (SDPA_INT k=0; k<mDim; k++){
    for (SDPA_INT l=0; l<A[k].SOCP_sp_nBlock; l++){
  ...
  NewArray(SOCP_constraint,SDPA_INT*,SOCP_nBlock);
  for (SDPA_INT l=0; l<SOCP_nBlock; l++){
    NewArray(SOCP_constraint[l],SDPA_INT,SOCP_nConstraint[l]);
  }
  NewArray(SOCP_blockIndex,SDPA_INT*,SOCP_nBlock);
  for (SDPA_INT l=0; l<SOCP_nBlock; l++){
    NewArray(SOCP_blockIndex[l],SDPA_INT,SOCP_nConstraint[l]);
  }

  // input index of non-zero block matrix of A
  NewArray(SOCP_count,SDPA_INT,SOCP_nBlock);
  for (SDPA_INT l=0; l<SOCP_nBlock; l++){
      SOCP_count[l] = 0;
  }
  for (SDPA_INT k=0; k<mDim; k++){
...
void InputData::initialize_index_LP()
{
  ...
  NewArray(LP_nConstraint,SDPA_INT,LP_nBlock);
  ...
  for (SDPA_INT k=0; k<mDim; k++){
    for (SDPA_INT l=0; l<A[k].LP_sp_nBlock; l++){
  ...
  NewArray(LP_constraint,SDPA_INT*,LP_nBlock);
  for (SDPA_INT l=0; l<LP_nBlock; l++){
    NewArray(LP_constraint[l],SDPA_INT,LP_nConstraint[l]);
  }
  NewArray(LP_blockIndex,SDPA_INT*,LP_nBlock);
  for (SDPA_INT l=0; l<LP_nBlock; l++){
    NewArray(LP_blockIndex[l],SDPA_INT,LP_nConstraint[l]);
  }

  // input index of non-zero block matrix of A
  NewArray(LP_count,SDPA_INT,LP_nBlock);
  for (SDPA_INT l=0; l<LP_nBlock; l++){
    LP_count[l] = 0;
  }
  for (SDPA_INT k=0; k<mDim; k++){
				      ...
				      Vector& retVec)
{
  ...
  retVec.setZero();
  for (SDPA_INT i=0; i<retVec.nDim; i++){
    Lal::let(ip,'=',A[i],'.',xMat);
    retVec.ele[i] = ip;
  }    
}
...
{
  retMat.setZero();
  for (SDPA_INT i=0; i<xVec.nDim; i++){
...
void InputData::display_index(FILE* fpout)
{
  ...
  printf("display_index: %ld %ld %ld\n",SDP_nBlock,SOCP_nBlock,LP_nBlock);

  for (SDPA_INT l=0; l<SDP_nBlock; l++){
    printf("SDP:%ldth block\n",l);
    for (SDPA_INT k=0; k<SDP_nConstraint[l]; k++){
      printf("constraint:%ld block:%ld \n",
	     SDP_constraint[l][k],SDP_blockIndex[l][k]);
    }
  }

  for (SDPA_INT l=0; l<SOCP_nBlock; l++){
    printf("SOCP:%ldth block\n",l);
    for (SDPA_INT k=0; k<SOCP_nConstraint[k]; k++){
      printf("constraint:%ld block:%ld \n",
  ...
  for (SDPA_INT l=0; l<LP_nBlock; l++){
    printf("LP:%ldth block\n",l);
    for (SDPA_INT k=0; k<LP_nConstraint[l]; k++){
      printf("constraint:%ld block:%ld \n",
...
Residuals::Residuals(SDPA_INT m, BlockStruct& bs,
...
void Residuals::initialize(SDPA_INT m, BlockStruct& bs,
			   InputData& inputData, Solutions& currentPt)
{
...
{
  double ret = 0.0;
  for (SDPA_INT k=0; k<primalVec.nDim; ++k) {
...
{
  SDPA_INT SDP_nBlock  = dualMat.SDP_nBlock;
  SDPA_INT SOCP_nBlock = dualMat.SOCP_nBlock;
  SDPA_INT LP_nBlock   = dualMat.LP_nBlock;
  double ret = 0.0;
  double tmp;

  for (SDPA_INT l=0; l<SDP_nBlock; ++l) {
    double* target = dualMat.SDP_block[l].de_ele;
    SDPA_INT size = dualMat.SDP_block[l].nRow;
    for (SDPA_INT j=0; j<size*size; ++j) {
      tmp = fabs(target[j]);
      if (tmp > ret) {
	ret = tmp;
      }
...
void Residuals::update(SDPA_INT m,
		       InputData& inputData,
		       Solutions& currentPt,
		       ComputeTime& com)
{
...
void Residuals::compute(SDPA_INT m,
			InputData& inputData,
			Solutions& currentPt)
{
  // p[k] = b[k] - A[k].X;
...
WorkVariables::WorkVariables(SDPA_INT m, BlockStruct& bs)
{
  initialize(m,bs);
}
...
void WorkVariables::initialize(SDPA_INT m, BlockStruct& bs)
{
  DLS1.initialize(bs);
  DLS2.initialize(bs);
  // DV1.initialize(m);
  ...
  if (bs.SDP_nBlock > 0){
    ...
    SDPA_INT* workStruct;
    NewArray(workStruct,SDPA_INT,bs.SDP_nBlock);
    for (SDPA_INT l=0; l<bs.SDP_nBlock; ++l) {
      workStruct[l] = max((SDPA_INT)1,3*bs.SDP_blockStruct[l]-1);
