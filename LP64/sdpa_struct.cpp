...
Vector::Vector(SDPA_INT nDim, double value)
{
  ele  = NULL;
...
void Vector::initialize(SDPA_INT nDim,double value)
{
  // rMessage("Vector initialize");
  if (nDim<=0) {
    rError("Vector:: nDim is nonpositive");
  }
  if ( ele != NULL )                  // Marc Pfetsch
  {
     if ( this->nDim != nDim )
     {
        DeleteArray(ele);
        NewArray(ele,double,nDim);    // Marc Pfetsch
     }
  }
  else
     NewArray(ele,double,nDim);       // Marc Pfetsch
...
BlockVector::BlockVector(SDPA_INT nBlock, SDPA_INT* blockStruct,
			   double value)
{
...
void BlockVector::initialize(SDPA_INT nBlock, SDPA_INT* blockStruct,
			      double value)
{
  ...
  this->nBlock = nBlock;
  NewArray(this->blockStruct,SDPA_INT,nBlock);
...
bool BlockVector::copyFrom(BlockVector& other)
{
  ...
  if (blockStruct==NULL) {
    nBlock = other.nBlock;
    NewArray(blockStruct,SDPA_INT,nBlock);
    for (SDPA_INT l=0; l<nBlock; ++l) {
...
SparseMatrix::SparseMatrix(SDPA_INT nRow, SDPA_INT nCol,
...
void SparseMatrix::initialize(SDPA_INT nRow, SDPA_INT nCol,
			      SparseMatrix::Type type,
			      SDPA_INT NonZeroNumber,
			      SparseMatrix::dsType DataStruct)
{
  ...
  case SPARSE:
	...
	if (DataStruct == DSarrays) {
	  NewArray(row_index,SDPA_INT,NonZeroNumber);
	  NewArray(column_index,SDPA_INT,NonZeroNumber);
...
void SparseMatrix::display(FILE* fpout, char* printFormat)
{
  ...
  case SPARSE:
    fprintf(fpout,"{");
    for (SDPA_INT index=0; index<NonZeroCount; ++index) {
      if (DataStruct == DSarrays) {
	i        = row_index[index];
...
void SparseMatrix::changeToDense(bool forceChange)
{
  ...
  type = DENSE;
  de_ele = NULL;
  SDPA_INT length = nRow*nCol;
  NewArray(de_ele,double,length);
  sdpa_dset(length,DZERO,de_ele,IONE);
  // all elements are set 0.
  for (SDPA_INT index=0; index<NonZeroCount; ++index) {
#if DATA_CAPSULE
    SDPA_INT        i = DataS[index].vRow;
    SDPA_INT        j = DataS[index].vCol;
    double value = DataS[index].vEle;
#else
    SDPA_INT        i = row_index[index];
    SDPA_INT        j = column_index[index];
...
bool SparseMatrix::sortSparseIndex(SDPA_INT& i, SDPA_INT& j)
{
  ...
  case SPARSE:
    ...
    for (SDPA_INT i1=0; i1<NonZeroCount; ++i1) {
#if DATA_CAPSULE
      SDPA_INT tmpi = DataS[i1].vRow;
      SDPA_INT tmpj = DataS[i1].vCol;
      if (tmpi>tmpj) {
	DataS[i1].vRow = tmpj;
	DataS[i1].vCol = tmpi;
      }
#else
      SDPA_INT tmpi = row_index[i1];
      SDPA_INT tmpj = column_index[i1];
      if (tmpi>tmpj) {
	row_index   [i1] = tmpj;
    ...
    // simple sort
    for (SDPA_INT i1=0; i1<NonZeroCount; ++i1) {
      for (SDPA_INT i2=0; i2<i1; ++i2) {
	#if DATA_CAPSULE
	SDPA_INT index1 = DataS[i1].vRow + DataS[i1].vCol;
	SDPA_INT index2 = DataS[i2].vRow + DataS[i2].vCol;
	if (index1<index2) {
	  SDPA_INT         tmpi = DataS[i2].vRow;
	  SDPA_INT         tmpj = DataS[i2].vCol;
	...
	#else
	SDPA_INT index1 = row_index[i1]+nCol*column_index[i1];
	SDPA_INT index2 = row_index[i2]+nCol*column_index[i2];
	if (index1<index2) {
	  SDPA_INT         tmpi = row_index   [i2];
	  SDPA_INT         tmpj = column_index[i2];
    ...
    for (SDPA_INT i1=0; i1<NonZeroCount-1; ++i1) {
#if DATA_CAPSULE
      SDPA_INT index1 = DataS[i1].vRow + DataS[i1].vCol;
      SDPA_INT index2 = DataS[i1+1].vRow + DataS[i1+1].vCol;
      if (index1 == index2) {
	...
	// remove redudunt
	for (SDPA_INT i2 = i1+1; i2<NonZeroCount-2;++i2) {
...
#else
      SDPA_INT index1 = row_index[i1  ]+nCol*column_index[i1  ];
      SDPA_INT index2 = row_index[i1+1]+nCol*column_index[i1+1];
      if (index1 == index2) {
...
DenseMatrix::DenseMatrix(SDPA_INT nRow, SDPA_INT nCol,
...
void DenseMatrix::initialize(SDPA_INT nRow, SDPA_INT nCol,
			     DenseMatrix::Type type)
{
  ...
  SDPA_INT old_length = this->nRow*this->nCol;
  this->nRow  = nRow;
  this->nCol  = nCol;
...
bool DenseMatrix::copyFrom(SparseMatrix& other)
{
  ...
  case SparseMatrix::SPARSE:
    ...
    length = nRow*nCol;
    sdpa_dset(length,DZERO,de_ele,IONE);
    for (SDPA_INT index = 0; index<other.NonZeroCount; ++index) {
      #if DATA_CAPSULE
      SDPA_INT i = other.DataS[index].vRow;
      SDPA_INT j = other.DataS[index].vCol;
      double value = other.DataS[index].vEle;
      #else
      SDPA_INT i = other.row_index[index];
      SDPA_INT j = other.column_index[index];
...
SparseLinearSpace::SparseLinearSpace(SDPA_INT SDP_nBlock, 
				     SDPA_INT* SDP_blockStruct,
				     SDPA_INT* SDP_NonZeroNumber,
				     SDPA_INT SOCP_nBlock, 
				     SDPA_INT* SOCP_blockStruct,
...
SparseLinearSpace::SparseLinearSpace(SDPA_INT SDP_sp_nBlock, 
...
void SparseLinearSpace::initialize(SDPA_INT SDP_nBlock, 
				   SDPA_INT* SDP_blockStruct,
				   SDPA_INT* SDP_NonZeroNumber,
				   SDPA_INT SOCP_nBlock, 
				   SDPA_INT* SOCP_blockStruct,
				   ...
				   bool* LP_NonZeroNumber)
{
  ...
  if (SDP_sp_nBlock > 0){
    NewArray(SDP_sp_index,SDPA_INT,SDP_sp_nBlock);
  ...
  for (SDPA_INT l=0; l<SDP_nBlock; ++l) {
    if (SDP_NonZeroNumber[l] > 0){
      ...
      SDPA_INT size = SDP_blockStruct[l];
  ...
  if (SOCP_sp_nBlock > 0){
    NewArray(SOCP_sp_index,SDPA_INT,SOCP_sp_nBLock);
  ...
  for (SDPA_INT l=0; l<SOCP_nBlock; ++l) {
    if (SOCP_NonZeroNumber[l] > 0){
      ...
      SDPA_INT size = SOCP_blockStruct[l];
  ...
  if (LP_sp_nBlock > 0){
    NewArray(LP_sp_index,SDPA_INT,LP_sp_nBlock);
...
void SparseLinearSpace::initialize(SDPA_INT SDP_sp_nBlock, 
                                   SDPA_INT* SDP_sp_index,
                                   SDPA_INT* SDP_sp_blockStruct, 
                                   SDPA_INT* SDP_sp_NonZeroNumber,
                                   SDPA_INT SOCP_sp_nBlock, 
                                   ...
                                   SDPA_INT* LP_sp_index)
{
  ...
  if (SDP_sp_nBlock > 0){
    NewArray(this->SDP_sp_index,SDPA_INT,SDP_sp_nBlock);
  ...
  for (SDPA_INT l=0; l<SDP_sp_nBlock; ++l) {
    ...
    SDPA_INT size = SDP_sp_blockStruct[l];
  ...
  if (SOCP_sp_nBlock > 0){
    NewArray(this->SOCP_sp_index,SDPA_INT,SOCP_sp_nBlock);
  ...
  for (SDPA_INT l=0; l<SOCP_sp_nBlock; ++l) {
    ...
    SDPA_INT size = SOCP_sp_blockStruct[l];
  ...
  if (LP_sp_nBlock > 0){
    NewArray(this->LP_sp_index,SDPA_INT,LP_sp_nBlock);
...
void SparseLinearSpace::setElement_SDP(SDPA_INT block,
				       SDPA_INT i, SDPA_INT j, double ele)
{
  SDPA_INT l;
  ...
  // set element
  SDPA_INT count = SDP_sp_block[l].NonZeroCount;
  #if DATA_CAPSULE
...
void SparseLinearSpace::setElement_SOCP(SDPA_INT block, SDPA_INT i, SDPA_INT j,
					double ele)
{
  rError("DenseLinearSpace:: current version does not support SOCP");
}

void SparseLinearSpace::setElement_LP(SDPA_INT block, double ele)
{
  SDPA_INT l;
...
bool SparseLinearSpace::sortSparseIndex(SDPA_INT& l, SDPA_INT& i, SDPA_INT& j)
{
  ...
  // for SDP
  if (SDP_sp_nBlock>0 && SDP_sp_index && SDP_sp_block) {
    for (SDPA_INT l_in=0; l_in<SDP_sp_nBlock; ++l_in) {
  ...
  l = -1;
  if (SOCP_sp_nBlock>0 && SOCP_sp_index && SOCP_sp_block) {
    for (SDPA_INT l_in=0; l_in<SOCP_sp_nBlock; ++l_in) {
...
void DenseLinearSpace::initialize(BlockStruct& bs)   // complete function - Marc Pfetsch
{
   // rMessage("DenseLinearSpace::initialize");
   if ( bs.SDP_nBlock + bs.SOCP_nBlock + bs.LP_nBlock <= 0) {
   ...
   if ( SDP_block != NULL )
   {
      if ( this->SDP_nBlock != bs.SDP_nBlock )
      {
         DeleteArray(SDP_block);
         if ( bs.SDP_nBlock > 0)
            NewArray(SDP_block,DenseMatrix,bs.SDP_nBlock);
      }
   }
   else
   {
   ...
   for (SDPA_INT l=0; l<SDP_nBlock; ++l) {
      SDPA_INT size = bs.SDP_blockStruct[l];
      if (size>0) {
   ...
   if ( SOCP_nBlock != NULL )
   {
      if ( this->SOCP_nBlock != bs.SOCP_nBlock )
      {
         DeleteArray(SOCP_block);
         if ( bs.SOCP_nBlock > 0)
            NewArray(SOCP_block,DenseMatrix,bs.SOCP_nBlock);
      }
   }
   else
   {
   ...
   if ( LP_block != NULL )
   {
      if ( this->LP_nBlock != bs.LP_nBlock )
      {
         DeleteArray(LP_block);
         if ( bs.LP_nBlock > 0)
            NewArray(LP_block,double,bs.LP_nBlock);
      }
   }
   else
   {
				       ...
				       char* printFormat)
{
  ...
  for (SDPA_INT l=0; l<bs.nBlock; l++){
    if (bs.blockType[l] == BlockStruct::btSDP) {
      ...
      for (SDPA_INT l2=0; l2<size-1; ++l2) {
...
void DenseLinearSpace::setElement_SDP(SDPA_INT block, SDPA_INT i, SDPA_INT j, double ele)
{
  ...
  SDPA_INT nCol = SDP_block[block].nCol;
  SDP_block[block].de_ele[i + j * nCol] = ele;
  SDP_block[block].de_ele[j + i * nCol] = ele;
}

void DenseLinearSpace::setElement_SOCP(SDPA_INT block, SDPA_INT i, SDPA_INT j,
				       double ele)
{
  rError("DenseLinearSpace:: current version does not support SOCP");
}

void DenseLinearSpace::setElement_LP(SDPA_INT block, double ele)
{
  // check range
  if (block >= LP_nBlock){
    rError("out of range in input data");
