...
void Chordal::mergeArray(SDPA_INT na1, SDPA_INT* array1, SDPA_INT na2, SDPA_INT* array2)
{

  SDPA_INT ptr = na1 + na2 - 1;
  SDPA_INT ptr1 = na1-1;
  SDPA_INT ptr2 = na2-1;
  SDPA_INT idx1, idx2;
...
void Chordal::catArray(SDPA_INT na1, SDPA_INT* array1, SDPA_INT na2, SDPA_INT* array2)
{
  SDPA_INT ind1 = 0;
  for (SDPA_INT index=0; index<na1; ++index) {
...
void Chordal::slimArray(SDPA_INT j, SDPA_INT length, SDPA_INT* array, SDPA_INT& slimedLength)
{
  if (length == 0) {
    return;
  }
...
void Chordal::makeGraph(InputData& inputData, SDPA_INT m)
{

  SDPA_INT j,k,l;
  SDPA_INT SDP_nBlock  = inputData.SDP_nBlock;
  SDPA_INT SOCP_nBlock = inputData.SOCP_nBlock;
  SDPA_INT LP_nBlock   = inputData.LP_nBlock;
  SDPA_INT* counter;
  NewArray(counter,SDPA_INT,m);
  for (j=0; j<m; j++){
    counter[j] = 0;
  }
  ...
  for (l = 0; l<SDP_nBlock; l++){
    SDPA_INT SDP_nConstraint = inputData.SDP_nConstraint[l];
  ...
  for (l = 0; l<SOCP_nBlock; l++){
    SDPA_INT SOCP_nConstraint = inputData.SOCP_nConstraint[l];
  ...
  for (l = 0; l<LP_nBlock; l++){
    SDPA_INT LP_nConstraint = inputData.LP_nConstraint[l];
  ...
  SDPA_INT** tmp;
  NewArray(tmp,SDPA_INT*,m);
  for (j=0; j<m; j++){
    // Anyway B(j,j) exists even if A(j) is empty
    counter[j]++;
  }
  for (j=0; j<m; j++){
    NewArray(tmp[j],SDPA_INT,counter[j]);
  }
  for (j=0; j<m; j++){
    // Anyway B(j,j) exists even if A(j) is empty
    tmp[j][0] = j;
  ...
  for (j=0; j<m; j++){
    ...
    for (SDPA_INT index=0; index<counter[j]; ++index) {
  ...
  SDPA_INT indexNZ = 0;
  for (j=0; j<m; ++j) {
    for (SDPA_INT index_i=0; index_i<counter[j]; ++index_i) {
...
double Chordal::analysisAndcountLowerNonZero(SDPA_INT m)
{
...
void Chordal::ordering_bMat(SDPA_INT m, SDPA_INT nBlock,
                            InputData& inputData,
			    FILE* Display, FILE* fpOut)
{
  ...
  #if 1
  for (SDPA_INT b=0; b<inputData.SDP_nBlock; b++){
    if (inputData.SDP_nConstraint[b] > m * sqrt(aggregate_threshold)){
      best = SELECT_DENSE;
      return;
    }      
  }
  for (SDPA_INT b=0; b<inputData.SOCP_nBlock; b++){
    if (inputData.SOCP_nConstraint[b] > m * sqrt(aggregate_threshold)){
      best = SELECT_DENSE;
      return;
    }      
  }
  for (SDPA_INT b=0; b<inputData.LP_nBlock; b++){
  ...
  SDPA_INT NonZeroAggregate = sparse_bMat_ptr->NonZeroCount*2-m;
...
#if PrintSparsity
  /* prlong int sparsity information */
  if (Display) {
    ...
    #else
    fprintf(Display, "Full Schur Elements %d, %.2e\n",
  ...
  if (fpOut) {
    ...
    fprintf(fpOut,
	    ...
	    mumps_id.infog[17-1],(double)mumps_id.infog[17-1]/1024);
  #else
    fprintf(fpOut, "Full Schur Elements Number %d, %.2e\n",
    	    (SDPA_INT)((double)m*m),(double)m*m);
...
bool Chordal::factorizeSchur(SDPA_INT m, SDPA_INT* diagonalIndex,
			     FILE* Display, FILE* fpOut)
{
